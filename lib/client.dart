  ...
  Future init() async {
    if (!initialized) {
      ...
      if (!kIsWeb) {
        ...
        var storage = FileStorage(cookieDir.path)
        ...
        cookieJar = new PersistCookieJar(storage: storage);
        this.http.interceptors.add(CookieManager(cookieJar));
        PackageInfo packageInfo = await PackageInfo.fromPlatform();
        addHeader('Origin',
            'appwrite-$type://${packageInfo.packageName ?? packageInfo.appName}');
      } else {
  ...
  Future<Response> call(HttpMethod method,
    ...
    try {
      ...
      throw AppwriteException(
          e.response.data['message'], e.response.data['code'], e.response.data);
    } catch (e) {
      throw AppwriteException(e.message);
    }
  }
